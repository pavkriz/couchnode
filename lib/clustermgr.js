    ...
    if (resp.statusCode !== 202) {
      try {
        ...
        if (errData._) {
          ...
          errMessage = Object.keys(errData.errors).map(function(errKey) { return errData.errors[errKey]; }).join(' ');
        } else if (errData !== null && typeof errData === 'object') {
          // join values (messages) of all properties in errData object
          errMessage = Object.keys(errData).map(function(errKey) { return errData[errKey]; }).join(' ');
        }
        var errObj = new Error(errMessage);
        // add full response and status code to the error
        errObj.couchResponse = errData;
        errObj.couchStatusCode = resp.statusCode;
